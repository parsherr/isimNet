...
import { Payment } from "@/lib/customers";
...
export default function EditPaymentModal({ open, onClose, onSubmit, initialPayment, maxAmount }: EditPaymentModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setAmountDisplay(formatCurrencyDisplay(String(initialPayment.amount)));
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-3xl px-5 pt-5 pb-8">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-5">Tahsilatı Düzenle</h2>
